...
const disabled = require("../../Schemas.js/Panel/Systems/remind");
...
module.exports = {
    ...
    async execute (interaction, client) {
        ...
        if (!data) return await interaction.reply({ content: `No **reminder** found with the **ID** of "**${id}**"!`, ephemeral: true});
        else {
            await interaction.reply({ content: `Your **reminder** with the **ID** of "**${id}**" has been **cancelled**!`, ephemeral: true});
        ...
        if (!alldata) return await interaction.reply({ content: `You **have not** set up any **reminders** yet!`, ephemeral: true });
            ...
            await interaction.reply({ content: `**All** of your **reminders** have been **cancelled**!`, ephemeral: true});
